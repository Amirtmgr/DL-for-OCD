...
def __save_state(state:State):
    #todo: check
    filename = cl.config.architecture.name + "_" + datetime.datetime.now() + '.pth'
    
    path = cl.config.models_path
    ...
    print(f"Model saved at {path+filename}")
...
def __run_epoch(epoch, phase, data_loader, network, criterion, optimizer, lr_scheduler, device):
    ...
    for batch_idx, (X, y) in enumerate(data_loader):
        ...
        #Append Predictions
        epoch_preds.append(prediction.detach().cpu())
    ...
    #Calculate metrics
    metrics = Metrics(averaging='macro')
...
def train_model(network, criterion, optimizer, lr_scheduler, train_loader, val_loader,device):
    ...
    try:
        epochs = int(cl.config.train.epochs)
    ...
    for epoch in tqdm(range(epochs),desc="Training model:"):
        ...
        # save best model
        if val_metrics.f1_score > best_f1_score: # type: ignore
